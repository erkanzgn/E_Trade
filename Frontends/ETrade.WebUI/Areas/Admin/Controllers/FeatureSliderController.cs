...
namespace ETrade.WebUI.Areas.Admin.Controllers
{
    ...
    public class FeatureSliderController : Controller
    {
        ...
        public async Task<IActionResult> CreateFeatureSlider(CreateFeatureSliderDto createFeatureSliderDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7074/api/FeatureSliders", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7074/api/FeatureSliders?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "FeatureSlider", new { area = "Admin" });
            }
        ...
        public async Task<IActionResult> UpdateFeatureSlider(string id)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7074/api/FeatureSliders/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateFeatureSlider(string id, UpdateFeatureSliderDto updateFeatureSliderDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7074/api/FeatureSliders/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
